...
Local reference-voice speech for HyperFrames on Windows. The release bundle supplies a versioned HyperFrames audio-engine override plus one CPU-only build of `llama-tts-server`.
...
- Deterministic segment caching keyed by text, reference audio, model identity, and generation settings.
...
```powershell
pwsh -NoProfile -File scripts/build-release.ps1 -Version 0.1.4
pwsh -NoProfile -File scripts/test-release.ps1 -Archive dist/hyperframes-voxcpm2-v0.1.4-windows-x64.zip
```
