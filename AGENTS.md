...
- `src/qwen3.mjs` owns the explicitly selectable Qwen CustomVoice provider and bounded native CLI/cache path. The exact Q8_0 community model uses the standalone GGML runtime, not llama.cpp. Presets only, German/Ryan defaults when selected, CPU-only. `patches/qwen3-tts-windows-cli.patch` owns Windows memory reporting and UTF-8 text/preset CLI integration.
...
- `src/supertonic.mjs` owns the default Supertonic/M1 provider and serialized subprocess/cache boundary. Qwen and VoxCPM2 require explicit selection. `src/supertonic-runner.py` adapts the official SDK without vendoring inference code. Supertonic uses original ONNX weights, CPU only, 16 threads, ten steps, no batching, and chunks of at most 300 characters.
