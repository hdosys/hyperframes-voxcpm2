...
- `scripts/build-release.ps1` owns the Windows x64 CPU and Vulkan release bundle.
...
- Run `npm test` and `pwsh -NoProfile -File scripts/test-source.ps1` before committing. A release additionally requires real CPU and Vulkan synthesis outside CI or an explicitly reported native-boundary blocker.
