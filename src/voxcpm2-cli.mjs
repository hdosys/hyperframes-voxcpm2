...
Options:
  ...
  --design DESCRIPTION Override the default Voice Design for this request.
  --voice FILE         Clone one explicit reference WAV instead of using Voice Design.
  --lang LANGUAGE      Language identity used for caching (default: en).
  --help               Show this help.
`;
...
export function parseVoxCPM2CLIArguments(argv, cwd = process.cwd()) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!["--text", "--output", "--design", "--voice", "--lang"].includes(token)) {
  ...
  return {
    ...
    voiceId: voice ? resolve(cwd, voice) : null,
